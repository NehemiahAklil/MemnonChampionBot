...
const randomChamp = (noncandidates, nominees, waitingInterval) => {
  let rand;
  let dif = dff - 1;
  ...
  if (!noAr.length) {
    ...
    console.log(rand);
    noAr.push({ name: rand, loop: 0 });
    // console.log(noAr);
  } else {
    // console.log('not first');
    arr = arr.filter((ar) => {
      if (!noAr.some((noA) => noA.name === ar)) {
        return arr;
      }
    });
    rand = arr[Math.floor(Math.random() * arr.length)];
    ...
    noAr = noAr
      .map((rands) => {
        if (rands.loop < dif) {
          return { name: rands.name, loop: rands.loop + 1 };
        } else {
          back = rands.name;
          return null;
        }
    ...
    // console.log(noAr);
    arr = arr.filter((value) => value !== rand);
...
for (let i = 0; i < 25; i++) {
  const { noAr } = randomChamp(fnoAr, farr, 3);
  fnoAr = noAr;
}
